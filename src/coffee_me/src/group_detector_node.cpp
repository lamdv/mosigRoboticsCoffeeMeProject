...
void update() {
    ...
    if ( init_laser && init_robot ) {
        ...
        if ( !current_robot_moving ) {
            ...
            detect_group_of_persons();//to detect group of persons using persons detected
...
void detect_group_of_persons(){
  ...
  for(int loop = 1; loop < nb_moving_persons_detected; loop++){
    ...
    if(min_person_id != -1){
      ...
      nb_of_persons_in_group[nb_groups_detected]++;
      nb_groups_detected++;
    }
  }
    

ROS_INFO("this is a test 2");

  float sum_of_x[nb_groups_detected], sum_of_y[nb_groups_detected];

  for(int i = 0; i<nb_groups_detected; i++){
      sum_of_x[i] = 0;
      sum_of_y[i] = 0;
  }
  ROS_INFO("this is a test 3");
...
//   }
    ...
    int group_to_go;
    group_to_go = round(rand()%nb_groups_detected);
    ROS_INFO("choosing group # %d", group_to_go);

    //update of the goal and publish of the goal
    goal_to_reach.x = groups_detected[group_to_go].x;
    goal_to_reach.y = groups_detected[group_to_go].y;
    goal_to_reach.z = 0;
    for(int i = 0; i<nb_groups_detected; i++){
    display[nb_pts].x = groups_detected[i].x;
    display[nb_pts].y = groups_detected[i].y;
