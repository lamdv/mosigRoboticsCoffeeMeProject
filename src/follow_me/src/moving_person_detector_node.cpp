...
void detect_motion() {
    ...
    //    if the difference between ( the background and the current value ) is higher than "detection_threshold"
        //then
        if((background[loop] >= range[loop] && (background[loop] - range[loop]) > detection_threshold) || (background[loop] < range[loop] && (range[loop] - background[loop]) > detection_threshold) ){
      //    ROS_INFO("MOTION DETECTION : loop = %d ---  backg = %d --- range = %d \n", loop, background[loop], range[loop]);
...
//else we start a new cluster with the current hit and end the current cluster
    ...
    for( int loop=1; loop<nb_beams; loop++ ){//loop over all the hits
      if(distancePoints(current_scan[loop], current_scan[loop-1]) <= cluster_threshold){
        ...
        //ROS_INFO("END cluster # %d,   size is : %d and dynamic = %d \n", nb_cluster, loop-cluster_start[nb_cluster], nb_dynamic);
        ...
        cluster_dynamic[nb_cluster]=(int)((100*nb_dynamic)/(clsize));
...
// - more than "dynamic_threshold"% of its hits are dynamic (see, cluster_dynamic table)
  ...
  //   ROS_INFO(" loop = %d ----  cluster size = %d    ----   dynamic = %d   ----\n", loop, cluster_size[loop], cluster_dynamic[loop]);
