...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
    ...
    */
        for (int loop = 0; loop < nb_beams; loop++)
        {
            if ((background[loop] - range[loop]) * (background[loop] - range[loop]) > detection_threshold * detection_threshold)
            {
                dynamic[loop] = 1;
            }
            else
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////*/
    void detect_moving_legs()
    {
        ...
        { //loop over all the clusters
            if ((distancePoints(current_scan[cluster_start[loop]], current_scan[cluster_end[loop]]) < leg_size_max) && (distancePoints(current_scan[cluster_start[loop]], current_scan[cluster_end[loop]]) > leg_size_min) && (cluster_dynamic[loop] > dynamic_threshold))
            {
        ...
        for (int loop_leg1 = 0; loop_leg1 < nb_moving_legs_detected; loop_leg1++)                 //loop over all the legs
            for (int loop_leg2 = loop_leg1 + 1; loop_leg2 < nb_moving_legs_detected; loop_leg2++) //loop over all the legs
                if (distancePoints(cluster_middle[loop_leg1], cluster_middle[loop_leg2]) < legs_distance_max)
                {
                    ...
                    moving_persons_detected[nb_moving_persons_detected].x = (cluster_middle[loop_leg1].x + cluster_middle[loop_leg2].x) / 2;
                    moving_persons_detected[nb_moving_persons_detected].y = (cluster_middle[loop_leg1].y + cluster_middle[loop_leg2].y) / 2;
